...
ui <- dashboardPage(
  ...
  dashboardHeader(title="Making Groups"),
  dashboardSidebar(
    sidebarMenu(
      menuItem("Groups",tabName = "groups",icon=icon("dashboard"))
    ),
...
server <- function(input,output){
  ...
  output$sheet <- DT::renderDataTable({groups()[,c(1,2,ncol(groups()))]})
  ...
  output$clusters <- renderPlot({
    ...
    km <- kmeans(data,nclus,nstart=25)
